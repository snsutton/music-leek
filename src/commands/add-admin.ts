...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!canManageAdmins(league, interaction.user.id)) {
    await interaction.reply({ content: 'Only the league creator can add admins!', flags: MessageFlags.Ephemeral });
    return;
  }

  if (isAdmin(league, targetUser.id)) {
    await interaction.reply({ content: `${targetUser.username} is already an admin!`, flags: MessageFlags.Ephemeral });
    return;
  }
